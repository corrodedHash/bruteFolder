...
class ChainTestClass(unittest.TestCase):
    ...
    def test_folding(self) -> None:
        ...
        self.assertListEqual(test_chain.chain, [0, 3, 0, 3, 2, 3, 2, 3])
    ...
    def test_reset_fold(self) -> None:
        ...
        test_chain.fold(3)
        self.assertListEqual(test_chain.chain, [0, 3, 0, 3])

        self.assertTrue(test_chain.can_fold(2))
        test_chain.fold(2)
        self.assertListEqual(test_chain.chain, [0, 3, 0])

        test_chain.reset_fold()
        self.assertListEqual(test_chain.chain, [0, 3, 0, 3])

        test_chain.fold(0)
        self.assertListEqual(test_chain.chain, [3, 0, 3])
